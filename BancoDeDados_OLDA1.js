    ...
    */
    ...
    CriaRelacionamento(master, detail, dados) {
      ...
      if (this.tabelas[master]) {
        // Garante que a entidade master é um array, se não for, transforma em um array
        const masterEntidade = Array.isArray(this.tabelas[master]) ? this.tabelas[master] : [this.tabelas[master]];
    
        // Pega os ids dos registros da tabela master
        const masterIds = masterEntidade.map(item => item.id);
    
        // Para cada item do detalhe, verifica se o masterId existe e cria o relacionamento
        dados.forEach(item => {
          if (masterIds.includes(item.id_grupo)) { // Alterado de masterId para id_grupo
    ...
    Destruir() {
      this.tabelas = null; // Remove os dados internos
      Object.keys(this).forEach(chave => delete this[chave]); // Remove propriedades
    }
    */
  }
